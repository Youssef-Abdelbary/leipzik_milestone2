...
import { fetchMyVendorOrders, updateDeliveryStatus, fetchMyVendorProfile, updateMyVendorProfile, fetchMyVendorInbox, respondToVendorRequest } from '../services/serviceBrowseVendors';
  ...
  if (loading) {
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
        {[1, 2].map(i => <div key={i} style={{ height: 220, background: P.surface, borderRadius: 14, border: `1px solid ${P.border}`, animation: `skpulse 1.4s infinite ${i * 0.12}s` }} />)}
      </div>
    );
  }
...
function ProfileEditor({ onSaved }) {
  ...
  if (loading) return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      {[1, 2, 3, 4].map(i => <div key={i} style={{ height: 120, background: P.surface, borderRadius: 14, border: `1px solid ${P.border}`, animation: `skpulse 1.4s infinite ${i * 0.12}s` }} />)}
    </div>
  );
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button
          onClick={handleSave}
          disabled={saving}
          style={{ padding: '11px 28px', background: `linear-gradient(135deg, ${P.blue} 0%, ${P.teal} 100%)`, border: 'none', borderRadius: 10, color: '#0a0a12', fontWeight: 700, fontSize: 14, cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: saving ? 0.7 : 1 }}
        >
          {saving ? 'Saving…' : 'Save Profile'}
        </button>
        {saved && <span style={{ fontSize: 13, color: P.teal, fontWeight: 600 }}>✓ Saved successfully</span>}
      </div>
    </div>
  );
}
...
export default function PageVendorDashboard() {
  ...
  useEffect(() => {
    ...
    if (tab && VENDOR_TABS.some(t => t.id === tab)) {
      switchTab(tab);
    }
  }, [searchParams]);
